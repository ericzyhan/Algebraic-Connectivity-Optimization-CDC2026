...
def _generate_figures(raw: "pd.DataFrame", summary_c: "pd.DataFrame | None",
    ...
    # Colorblind-friendly, vivid colours that work on both dark/light backgrounds
    COLORS: dict[int, str] = {
        8: "#E6194B", 16: "#3B75AF", 32: "#44AA44", 64: "#D55E00", 128: "#883EBB",
    }
    MARKERS: dict[int, str] = {8: "o", 16: "s", 32: "D", 64: "^", 128: "v"}

    DARK_RC: dict[str, object] = {
        "text.color": "#e0e0e0", "axes.labelcolor": "#e0e0e0",
        "axes.edgecolor": "#777777", "xtick.color": "#bbbbbb", "ytick.color": "#bbbbbb",
        "grid.color": "#e0e0e0", "grid.alpha": 0.10,
    ...
    LIGHT_RC: dict[str, object] = {
        ...
        "grid.color": "#444444", "grid.alpha": 0.18,
        "legend.facecolor": "none", "legend.edgecolor": "none",
        "figure.facecolor": "none", "axes.facecolor": "none",
        "figure.dpi": 150, "font.size": 11, "axes.titlesize": 13,
        "axes.labelsize": 12, "legend.fontsize": 10, "lines.linewidth": 1.5,
    ...
    # ── Fig 1: λ₂ vs ρ (BB+RL, all n overlaid) ──
    with plt.rc_context(DARK_RC):
        ...
        fig.tight_layout()
        fig.savefig(fig_dir / "fig1_lambda2_vs_rho.png", bbox_inches="tight", transparent=True)
        plt.close(fig)
        print("  [FIG] fig1_lambda2_vs_rho.png")
    ...
    with plt.rc_context(DARK_RC):
        ...
        ax.grid(True, alpha=0.10)
        fig.tight_layout()
        fig.savefig(fig_dir / "fig2_lambda2norm_vs_rho.png", bbox_inches="tight", transparent=True)
        plt.close(fig)
        print("  [FIG] fig2_lambda2norm_vs_rho.png")

    # ── Fig 3: BB+RL vs Path+RL ──
    with plt.rc_context(DARK_RC):
        ...
        for idx, n in enumerate(n_order):
            ...
            grp_p = group_agg(sub_p, "rho_target", "terminal_lambda2", ["rho_target"])
            ax.plot(grp_p["rho_target"], grp_p["mean"], "--", color="#999999", linewidth=1.2, label="Path+RL")
            sub_b = bb[bb["n"] == n]
            grp_b = group_agg(sub_b, "rho_target", "terminal_lambda2", ["rho_target"])
            ax.errorbar(grp_b["rho_target"], grp_b["mean"], yerr=grp_b["std"],
                        fmt=f"-{MARKERS[n]}", color=COLORS[n], capsize=3, markersize=5, label="Backbone+RL")
            ax.set_xlabel(r"$\rho$"); ax.set_ylabel(r"$\lambda_2$"); ax.set_title(f"$n={n}$")
            ax.legend(fontsize=8, framealpha=0.0); ax.grid(True, alpha=0.10)
        axes_flat[-1].set_visible(False)
        fig.suptitle(r"$\lambda_2$: Backbone+RL vs Path+RL", fontsize=14, y=1.02)
        fig.tight_layout()
        fig.savefig(fig_dir / "fig3_bb_vs_path.png", bbox_inches="tight", transparent=True)
        plt.close(fig)
        print("  [FIG] fig3_bb_vs_path.png")
    ...
    with plt.rc_context(DARK_RC):
        ...
        for idx, n in enumerate(n_order):
            ...
            ax.legend(fontsize=8, framealpha=0.0); ax.grid(True, alpha=0.10)
        axes_flat[-1].set_visible(False)
        fig.suptitle(r"Backbone+RL: $\lambda_2$ by Backbone Family", fontsize=14, y=1.02)
        fig.tight_layout()
        fig.savefig(fig_dir / "fig4_by_family.png", bbox_inches="tight", transparent=True)
        plt.close(fig)
        print("  [FIG] fig4_by_family.png")
    ...
    if summary_c is not None:
        with plt.rc_context(DARK_RC):
            ...
            ax.errorbar(sc["n"].values, sc["runtime_mean_over_density_s"].values,
                        ...
                        fmt="o-", color="#44BBDD", capsize=4, markersize=8, linewidth=2)
            for _, row in sc.iterrows():
                ax.annotate(f"$n={int(row['n'])}$", (row["n"], row["runtime_mean_over_density_s"]),
                            textcoords="offset points", xytext=(8, 8), fontsize=10,
                            color=DARK_RC["text.color"])
            ax.set_xlabel("Graph size $n$"); ax.set_ylabel("Mean runtime per episode (s)")
            ax.set_title("Backbone+RL: Runtime Scaling")
            ax.set_xscale("log", base=2); ax.set_yscale("log")
            ax.set_xticks(n_order); ax.set_xticklabels([str(v) for v in n_order])
            ax.grid(True, alpha=0.10, which="both")
            fig.tight_layout()
            fig.savefig(fig_dir / "fig5_runtime.png", bbox_inches="tight", transparent=True)
            plt.close(fig)
            print("  [FIG] fig5_runtime.png")
    ...
    with plt.rc_context(DARK_RC):
        ...
        fig.tight_layout()
        fig.savefig(fig_dir / "fig6_heatmap.png", bbox_inches="tight", transparent=True)
        plt.close(fig)
        print("  [FIG] fig6_heatmap.png")

    # ── MAIN RESULT (white-background readable) ──
    with plt.rc_context(LIGHT_RC):
        ...
        for n in n_order:
            ...
            grp = group_agg(sub, "rho_target", "terminal_lambda2", ["rho_target"])
            ax.errorbar(grp["rho_target"], grp["mean"], yerr=grp["std"],
                        fmt=f"-{MARKERS[n]}", color=COLORS[n], capsize=3, markersize=6,
                        linewidth=2, label=f"BB+RL $n={n}$")
        for n in [8, 16, 32, 64, 128]:
            sub_p = path_only[path_only["n"] == n]
            if sub_p.empty:
                continue
            grp_p = group_agg(sub_p, "rho_target", "terminal_lambda2", ["rho_target"])
            ax.plot(grp_p["rho_target"], grp_p["mean"], "--", color="#888888", linewidth=1.0, alpha=0.55)
        ...
        ax.set_title(r"Backbone+RL vs Path+RL: $\lambda_2$ across Graph Sizes and Densities", fontsize=14)
        ax.legend(handles=handles, labels=labels, ncol=2, loc="lower right",
                  fontsize=9, framealpha=0.85, edgecolor="#cccccc")
        ax.grid(True, alpha=0.18)
        fig.tight_layout()
        fig.savefig(fig_dir / "fig_main_result.png", bbox_inches="tight", transparent=True, dpi=200)
        plt.close(fig)
        print("  [FIG] fig_main_result.png (white-background readable)")
    ...
    with plt.rc_context(DARK_RC):
        ...
        fig.tight_layout()
        fig.savefig(fig_dir / "fig7_summary_panel.png", bbox_inches="tight", transparent=True)
        plt.close(fig)
        print("  [FIG] fig7_summary_panel.png")
