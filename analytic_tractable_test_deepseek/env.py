...
from .analytic_state import AnalyticState
from .calibration import ETA_NS, ETA_P_VALS, ETA_R_VALS
...
class GraphEnv:
    def __init__(
        ...
        reward_alpha_1: float = 0.10,
        reward_alpha_2: float = 0.20,
        reward_alpha_3: float = 0.70,
        reward_eta_p: float = 0.0,
    ):
        self.env_id = env_id
        ...
        self._analytic: Optional[AnalyticState] = None
        self._episode_init_metadata: Dict[str, Any] = default_path_init_metadata(0)

    # Calibrated η_R and η_p tables (generated by calibrate_eta.py).
    _ETA_TABLE_NS = np.asarray(ETA_NS, dtype=np.float64)
    _ETA_R_TABLE_VALS = np.asarray(ETA_R_VALS, dtype=np.float64)
    _ETA_P_TABLE_VALS = np.asarray(ETA_P_VALS, dtype=np.float64)

    def _interp_eta(self, table_vals: np.ndarray) -> float:
        """Linear interpolation of a calibrated per-n table, clipped to range."""
        n_f = float(max(2, self.n))
        ns = type(self)._ETA_TABLE_NS
        vs = np.asarray(table_vals, dtype=np.float64)
        if n_f <= float(ns[0]):
            return float(vs[0])
        if n_f >= float(ns[-1]):
            return float(vs[-1])
    ...
    def _effective_eta(self) -> float:
        """η_R for the current graph size n (calibrated; config override wins)."""
        if float(self.reward_eta) > 0.0:
            return float(self.reward_eta)
        return self._interp_eta(type(self)._ETA_R_TABLE_VALS)

    def _effective_eta_p(self) -> float:
        """η_p for the current graph size n (calibrated; config override wins)."""
        if float(self.reward_eta_p) > 0.0:
            return float(self.reward_eta_p)
        return self._interp_eta(type(self)._ETA_P_TABLE_VALS)

    def _init_analytic_state(self) -> Tuple[float, float, float, np.ndarray, np.ndarray, np.ndarray, float, int, np.ndarray]:
        """Create a fresh AnalyticState from the current adjacency.

        Returns the initial spectral features (same tuple as spectral_features).
        """
        if self.adj is None:
            raise RuntimeError("adjacency not set")
        self._analytic = AnalyticState(self.adj)
        return self._analytic.get_spectral_features()
        ...
        *,
        ...
        if init_mode == "backbone":
            ...
            if self.incremental_observation:
                self._initialize_incremental_state()
            spectral_tuple = self._init_analytic_state()
        ...
        *,
        ...
        if self.rl_variant == "full":
            ...
            if self._analytic is None:
                raise RuntimeError("Analytic state not available for full variant observation")
            analytic_pairs, _cand_info = self._analytic.select_candidates(
                top=32,
                alpha1=self.reward_alpha_1,
                alpha2=self.reward_alpha_2,
                alpha3=self.reward_alpha_3,
                eta_R=self._effective_eta(),
                eta_p=self._effective_eta_p(),
            )
            candidate_pairs, pair_features = build_candidate_features_full(
                ...
                candidate_pairs_override=analytic_pairs,
            )
        elif self.rl_variant == "lite_v2":
            if not _HAS_LITE:
                raise RuntimeError(
                    "lite_v2 variant requires features/lite.py, which is not present "
                    "in this package; use rl_variant='full'"
                )
    ...
    def step(
        ...
        if use_spectral:
            ...
            # Incremental analytic rank-1 update (algorithm.md Single-Edge Update)
            if self._analytic is not None:
                self._analytic.add_edge(i, j)
                spectral_tuple = self._analytic.get_spectral_features()
            else:
            ...
            # P_min from the analytic state (exact recomputation after add)
            if self._analytic is not None:
                new_P_min = self._analytic.get_P_min()
            else:
                new_P_min = old_P_min
    ...
    def load_state_dict(self, state: Dict) -> None:
        ...
        if self.incremental_observation:
            self._initialize_incremental_state()
        analytic_state = state.get("analytic_state") or state.get("spectral_tracker")
        if analytic_state is not None:
            self._analytic = AnalyticState.from_state_dict(analytic_state)
        else:
            self._analytic = None
