...
def build_candidate_features(
    *,
    ...
    candidate_pairs_override: Optional[np.ndarray] = None,
) -> tuple[np.ndarray, np.ndarray]:
    """Build pair features for candidate edges.

    If ``candidate_pairs_override`` is provided (an (m, 2) int array of node
    indices selected by the analytic algorithm), those exact pairs are used
    and the internal top-``top_k`` spectral-gap selection is skipped.
    Otherwise, fall back to the default λ₂-eigenspace-gap top-``top_k``
    selection over non-edges.
    """
    has_override = (
        candidate_pairs_override is not None
        and np.asarray(candidate_pairs_override).size > 0
    )
    if (
        incremental_observation
        ...
        if has_override:
            candidate_pairs = np.asarray(candidate_pairs_override, dtype=np.int64)
        elif nonedge_pair_idx.size == 0:
            return (
